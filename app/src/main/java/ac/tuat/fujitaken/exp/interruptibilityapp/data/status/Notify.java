...
public class Notify {
    ...
    private int noteCount = NOTE_TIME;  //s 新しい通知が配信されてからのカウント

    public Notify(Context context){
        appName = context.getString(R.string.app_name);
    }

    //s 新しい通知が配信されてから一定時間内かどうかを判定をする
    //s interruptTiming.run() から定期的に呼ばれる：引数で渡されるのは たぶん 最新の通知 っぽい
    public int judge(String note){
        noteCount = (!"".equals(note) && !appName.equals(note))? 0: noteCount + 1;  //s 新しい通知ではなかったら noteCount を 1 増加
        if(noteCount < NOTE_TIME){  //s 通知が配信されてから一定時間内
            return NOTIFICATION;
        }
